    	...
    	if(Request::instance()->isAjax()){
    		...
    		if(in_array('', $input)){
    			...
    			if(Db::table('auth_group_access')->where('uid',$input['user_id'])->update($data)){
    				$res = ['errno' => 1, 'message' => '修改角色成功'];
    			}
    		}else if(Db::table('auth_group_access')->insert($data)){
				$res = ['errno' => 1, 'message' => '分配角色成功'];
			}
    		return $res;
    	}
